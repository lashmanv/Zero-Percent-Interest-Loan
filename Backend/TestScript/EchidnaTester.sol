...
contract EchidnaTester {
    ...
    TroveManager1 public troveManager1;
    TroveManager2 public troveManager2;
    TroveManager3 public troveManager3;
    ...
    constructor() payable {
        troveManager1 = new TroveManager1();
        troveManager2 = new TroveManager2();
        troveManager3 = new TroveManager3();
        ...
        zusdToken = new ZUSDToken(
            address(troveManager1),
            address(troveManager2),
            address(troveManager3),
        ...
        troveManager1.setAddresses(address(borrowerOperations), address(troveManager2), 
        ...
        troveManager2.setAddresses(address(borrowerOperations), address(troveManager1), 
            address(activePool), address(defaultPool), 
            address(stabilityPool), address(gasPool), address(collSurplusPool),
            address(priceFeedTestnet), address(zusdToken), 
            address(sortedTroves), address(0), address(0));

        troveManager3.setAddresses(address(borrowerOperations), address(troveManager1), 
            address(activePool), address(defaultPool), 
            address(stabilityPool), address(gasPool), address(collSurplusPool),
            address(priceFeedTestnet), address(zusdToken), 
            address(sortedTroves), address(0), address(0));
       
        borrowerOperations.setAddresses(address(troveManager1), address(troveManager2), 
            address(troveManager3), address(activePool), address(defaultPool), 
            address(stabilityPool), address(gasPool), address(collSurplusPool),
        ...
        activePool.setAddresses(address(borrowerOperations), 
            address(troveManager1),address(troveManager2), address(troveManager3),  address(stabilityPool), address(defaultPool));

        defaultPool.setAddresses(address (borrowerOperations), address(troveManager1), address(troveManager2), address(troveManager3), address(activePool));
        
        stabilityPool.setAddresses(address(borrowerOperations), 
            address(troveManager1),address(troveManager2), address(troveManager3), address(activePool), address(zusdToken), 
            address(sortedTroves), address(priceFeedTestnet), address(0));

        collSurplusPool.setAddresses(address(borrowerOperations), 
             address(troveManager1), address(troveManager2), address(troveManager3), address(activePool));
    
        sortedTroves.setParams(1e18, address(troveManager1), address(troveManager2), address(troveManager3), address(borrowerOperations));

        for (uint i = 0; i < NUMBER_OF_ACTORS; i++) {
            echidnaProxies[i] = new EchidnaProxy( borrowerOperations, troveManager1, troveManager2, troveManager3, stabilityPool, zusdToken);
    ...
    function liquidateTrovesExt(uint _i, uint _collId, uint _n) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].liquidateTrovesPrx(_collId, _n);
    }
    ...
    function redeemCollateralExt(
        ...
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].redeemCollateralPrx(_collId, _ZUSDAmount, _firstRedemptionHint, _upperPartialRedemptionHint, _lowerPartialRedemptionHint, _partialRedemptionHintNICR, 0, 0);
    }
    ...
    function openTroveExt(uint _i, uint _ETH, uint _ZUSDAmount) public payable {
        ...
        numberOfTroves = troveManager1.getTroveOwnersCount();
    ...
    function echidna_troves_order(uint _collId) external view returns(bool) {
        address currentTrove = sortedTroves.getFirst(_collId);
        address nextTrove = sortedTroves.getNext(currentTrove, _collId);

        while (currentTrove != address(0) && nextTrove != address(0)) {
            if (troveManager1.getNominalICR(nextTrove) > troveManager1.getNominalICR(currentTrove)) {
                return false;
            }
            ...
            nextTrove = sortedTroves.getNext(currentTrove, _collId);
        }
     ...
     * Stake > 0
     */
    function echidna_trove_properties(uint _collId) public view returns(bool) {
        address currentTrove = sortedTroves.getFirst(_collId);
        while (currentTrove != address(0)) {
            // Status
            if (ITroveManager1.Status(troveManager1.getTroveStatus(currentTrove)) != ITroveManager1.Status.active) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;
            ...
            // Minimum debt (gas compensation)
            if (troveManager1.getTroveDebt(currentTrove) < ZUSD_GAS_COMPENSATION) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;
            ...
            // Stake > 0
            if (troveManager1.getTroveStake(currentTrove) == 0) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;

            currentTrove = sortedTroves.getNext(currentTrove, _collId);
        }
        return true;
    }

    function echidna_ETH_balances() public view returns(bool) {
        if (address(troveManager1).balance > 0) {
            return false;
        }
        ...
        uint[] memory ethBal = stabilityPool.getTokenBalances();
        if (address(stabilityPool).balance != ethBal[0]) {
            return false;
        }
    ...
    // Total ZUSD matches
    function echidna_ZUSD_global_balances(uint _collId) public view returns(bool) {
