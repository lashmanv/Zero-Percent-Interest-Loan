...
interface IStabilityPool {
    ...
    // --- Events ---
    event StabilityPoolBalanceUpdated(uint[] _newBalance);
    event StabilityPoolETHBalanceUpdated(uint _newBalance);
    event StabilityPoolTokenBalanceUpdated(uint _collId, uint _newBalance);
    ...
    event TokenGainWithdrawn(address indexed _depositor, uint[] _ETH, uint _ZUSDLoss);
    event ZQTYPaidToDepositor(address indexed _depositor, uint _ZQTY);
    event ZQTYPaidToFrontEnd(address indexed _frontEnd, uint _ZQTY);
    event TokenSent(address _to, uint _collId, uint _amount);
     ...
     * - Sends depositor's accumulated gains (ZQTY, ETH) to depositor
     ...
     */
    function withdrawTokenGainToTrove(address _upperHint, address _lowerHint) external;
     ...
     */
    function offset(uint _debt, uint _collId, uint _coll) external;
     ...
     */
    function getDepositorTokenGain(address _depositor) external view returns (uint[] memory);
     ...
     */
    function getDepositorZQTYGain(address _depositor) external view returns (uint);

    /*
     * Return the ZQTY gain earned by the front end.
     */
    function getFrontEndZQTYGain(address _frontEnd) external view returns (uint);
