...
contract LiquityBase is BaseMath, ILiquityBase {
    ...
    function getEntireSystemColl() public view returns (uint, uint[] memory, uint[] memory) {
        uint activeETHColl = activePool.getETH();
        uint[] memory activeTokenColl = activePool.getTokenBalances();
        uint liquidatedColl = defaultPool.getETH();
        uint[] memory liquidatedTokens = defaultPool.getTokenBalances();

        return (activeETHColl + liquidatedColl, activeTokenColl, liquidatedTokens) ;
    }
    ...
    function _getTCR(uint[] memory _price) internal view returns (uint) {
        ...
        uint TCR = LiquityMath._computeEntireCR(entireSystemDebt, entireSystemPrice);
