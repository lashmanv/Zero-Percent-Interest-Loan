...
/*
 * On deployment a new Uniswap pool will be created for the pair ZUSD/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of ZQTY token,
 ...
 * in order to avoid getting ZQTY tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either ZQTY token contract is deployed, and therefore ZQTY tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(zqtyToken.getLpRewardsEntitlement(), _duration);
    ...
    function claimReward() public override {
        ...
        zqtyToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == zqtyToken.balanceOf(address(this)));
