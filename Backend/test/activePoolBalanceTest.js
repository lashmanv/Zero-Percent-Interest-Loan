...
const ercToken = require ( "../client/src/artifacts/TestScript/AAAERC.sol/MyToken.json");
  ...
  describe('Zero loan Unit Tests', async function () {
    ...
    describe('Active Pool Sequence Liquidation', async function () {
      before(async () => {
        try {
      ...
      it("OpenTrove with 5 ETH and 1800 ZUSD", async () => {
        ...
        let afterBalance = await activePoolContract.getTokenBalances();

        expect(afterBalance[0]).to.equal(eth);
      ...
      it("OpenTrove with 1.25 ETH and 1800 ZUSD", async () => {
        ...
        let previousBalance = await activePoolContract.getTokenBalances();

        await openTrovewithEth(signerId, eth, zusd, upperHint, lowerHint);
        
        let afterBalance = await activePoolContract.getTokenBalances();
        expect(afterBalance[0]).to.equal(eth.add(previousBalance[0]));
      ...
      it("OpenTrove with 5 WETH Token and 1800 ZUSD", async () => {
        ...
        let afterBalance = await activePoolContract.getTokenBalances();
        expect(afterBalance[tokenId]).to.equal(token.add(previousBalance[tokenId]));
      ...
      it("Active Pool Token balance after 2 ETH Troves liquidation", async () => {  
          const tokenId = 0;
          const zusd = ethers.utils.parseUnits('1800')
          const expected = ethers.utils.parseUnits('5')

          await liquidateinSequence(tokenId, 2);
        
          let afterBalance = await activePoolContract.getTokenBalances();
          expect(afterBalance[tokenId]).to.equal(expected);
      ...
      it("Active Pool Token balance after 2 WETH Troves liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('5')
      ...
      it("Active Pool Token balance after 2 WBTC Troves liquidation", async () => {  
        const tokenId = 2;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('5')
      ...
      it("Active Pool Token balance after 2 WBNB Troves liquidation", async () => {  
        const tokenId = 3;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('10')
      ...
      it("Active Pool Token balance after 2 WSOL Troves liquidation", async () => {  
        const tokenId = 4;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('250')
      ...
      it("Active Pool Token balance after 2 USDC Troves liquidation", async () => {  
        const tokenId = 5;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('5000')
    ...
    describe('Stablility Pool Random Liquidation', async function () {
      ...
      it("Active Pool Token ZUSD balance after 1 WETH Trove liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('6.25')
      ...
      it("Active Pool Token ZUSD balance after 2 WBTC Troves liquidation", async () => {  
        const tokenId = 2;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('5')
      ...
      it("Active Pool Token ZUSD balance after 1 WETH Troves liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('5')
      ...
      //     console.log('');
      // })
      it("Active Pool Token ZUSD balance after 1 USDC Troves liquidation", async () => {  
        const tokenId = 5;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('7750')
      ...
      it("Active Pool Token ZUSD balance after 2 WSOL Troves liquidation", async () => {  
        const tokenId = 4;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('250')
      ...
      it("Active Pool Token ZUSD balance after 2 WBNB Troves liquidation", async () => {  
        const tokenId = 3;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('10')
