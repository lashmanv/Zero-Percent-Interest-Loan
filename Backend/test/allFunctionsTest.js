...
const ercToken = require ( "../client/src/artifacts/TestScript/AAAERC.sol/MyToken.json");
  ...
  describe('Zero loan Unit Tests', async function () {
    ...
    async function closeTrove(signerId) {
        try {
          const zusd = ethers.utils.parseUnits('9')
  
          await zusdTokenContract.connect(signers[signerId]).unprotectedMint(signers[signerId].address, zusd);
          await borrowerOperationsContract.connect(signers[signerId]).closeTrove();
        }
        catch(error) {
    ...
    async function withdraw() {
        try {  
          let p = ethers.utils.parseUnits('4000');

          await price();
          await stabilityPoolContract.connect(signers[19]).withdrawFromSP(p);
        }
        catch(error) {
          console.log(error.reason);
          return(error.reason);
    ...
    async function price() {
      ...
      async function redeemHint(tokenId, zusd) {
        try{   
            // Get the redemptions hints from the deployed HintHelpers contract
            price = await priceFeedContract.fetchEntirePrice();
            // zusd = ethers.utils.parseEther('1810');
            const redemptionhint = await hintHelpersContract.getRedemptionHints(tokenId, zusd, price[tokenId], 50);

            const { 0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedLUSDAmount } = redemptionhint

            let numTroves = await sortedTrovesContract.getSize(tokenId);
            let numTrials = numTroves.mul(15);

            // Get the approximate partial redemption hint
            const { hintAddress: approxPartialRedemptionHint } = await hintHelpersContract.getApproxHint(tokenId, partialRedemptionNewICR, numTrials, 42)
            ...
            */
            const exactPartialRedemptionHint = (await sortedTrovesContract.findInsertPosition(tokenId,partialRedemptionNewICR, approxPartialRedemptionHint, approxPartialRedemptionHint))

            const maxFee = '5'.concat('0'.repeat(16)) // Slippage protection: 5%
        
            const returnValue = [truncatedLUSDAmount, firstRedemptionHint, exactPartialRedemptionHint[0], exactPartialRedemptionHint[1], partialRedemptionNewICR];
    ...
    async function redeem(tokenId, zusdAmount) {
        let fee = BigInt(500000000000000000)

        try {
            const hints = await redeemHint(tokenId, zusdAmount);

            // console.log(tokenId, hints[0], hints[1], hints[2], hints[3], hints[4]);
            await troveManager2Contract.connect(signers[19]).redeemCollateral(tokenId, hints[0], hints[1], hints[2], hints[3], hints[4], 0, fee);
        }
        catch(error) {
            console.log(error);
            return(error);
    ...
    describe('All functions', async function () {
      before(async () => {
        try {
      ...
      it("CloseTrove with 1.5 WBTC Token and 1800 ZUSD", async () => {
        const signerId = 8;
        const tokenId = 2;
        const token = ethers.utils.parseUnits('1.5')
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await closeTrove(signerId);
        let afterBalance = await zusdTokenContract.balanceOf(signers[signerId].address);
      ...
      it("Stability Pool ZQTY gain after liquidation", async () => {  
        ...
        it("Stability Pool ZUSD balance after 2 ETH Trove liquidation", async () => {  
            const tokenId = 0;
            const zusd = ethers.utils.parseUnits('1800')
            const expected = ethers.utils.parseUnits('2009')
        ...
        it("Stability Pool Withdraw ZUSD after liquidation", async () => {  
            const tokenId = 0;
            const zusd = ethers.utils.parseUnits('1800')
            const expected = ethers.utils.parseUnits('4000')
        ...
        it("Redeem WSOL for 1809 ZUSD", async () => {  
            const tokenId = 4;
            const zusd = ethers.utils.parseUnits('1809');
            const prevEx = await zusdTokenContract.balanceOf(signers[19].address);;
            const afterEx = prevEx.sub(zusd);

            await redeem (tokenId, zusd);

            let afterBalance = await zusdTokenContract.balanceOf(signers[19].address);
            expect(afterBalance).to.equal(afterEx);

            let balance = await ercToken4Contract.connect(signers[19]).balanceOf(signers[19].getAddress());
