...
const ercToken = require ( "../client/src/artifacts/TestScript/AAAERC.sol/MyToken.json");
  ...
  describe('Zero loan Unit Tests', async function () {
    ...
    async function liquidateinBatch(troveId) {
      try {
        
        let arr = [];
        let i = 0;
        for(i = 0; i < troveId.length; i++) {
          arr[i] = (await signers[troveId[i]].getAddress());
        }

        // const troveArray = JSON.stringify(arr).replace(/'/g, '"');

        // console.log(arr);

        // console.log(troveArray);

        await troveManager3Contract.connect(signers[19]).batchLiquidateTroves(arr);
      }
      catch(error) {
        console.log(error);
        return(error);
    ...
    describe('Batch Liquidation', async function () {
      before(async () => {
        try {
      ...
      it("Set Price", async () => {  
        ...
        let e = ethers.utils.parseUnits('1500');

        expect(await priceFeedContract.fetchCollPrice(0)).to.equal(e);
        
        console.log('');
    })

      // it("Revert when Liquidate all the ETH Troves", async () => {  
      //     const tokenId = 0;

      //     let previousBalance = await zusdTokenContract.balanceOf(signers[19].address);
      //     expect(previousBalance).to.equal(0);

      //     let error = await liquidateinBatch(tokenId, 3);
      ...
      it("Batch Liquidate 2 ETH Troves", async () => {  
          const troveArray = [2,3];
          const zusd = ethers.utils.parseUnits('400')

          let previousBalance = await zusdTokenContract.balanceOf(signers[19].address);
          expect(previousBalance).to.equal(0);

          await liquidateinBatch(troveArray);
      ...
      // it("Revert when Liquidate all the WETH Troves", async () => {  
      //     const tokenId = 1;
      ...
      it("Batch Liquidate 2 WETH Troves", async () => {  
        const troveArray = [5,6];
      ...
      // it("Revert when Batch Liquidate all the WBTC Troves", async () => {  
      //     const tokenId = 2;
      ...
      it("Batch Liquidate 2 WBTC Troves", async () => {  
          const troveArray = [8,9];
      ...
      // it("Revert when Batch Liquidate all the WBNB Troves", async () => {  
      //     const tokenId = 3;
      ...
      it("Batch Liquidate 2 WBNB Troves", async () => {  
          const troveArray = [11,12];
      ...
      // it("Revert when Batch Liquidate all the WSOL Troves", async () => {  
      //     const tokenId = 4;
      ...
      it("Batch Liquidate 2 WSOL Troves", async () => {  
          const troveArray = [14,15];
      ...
      // it("Revert when Batch Liquidate all the USDC Troves", async () => {  
      //     const tokenId = 5;
      ...
      it("Batch Liquidate 2 USDC Troves", async () => {  
          const troveArray = [17,18];
    ...
    describe('Random Liquidation', async function () {
      ...
      // it("Batch Liquidate with same price ETH Troves", async () => {  
      //     const tokenId = 0;
    ...
    it("Set Price", async () => {  
      ...
      // it("Revert when Batch Liquidate all the ETH Troves", async () => {  
      //     const tokenId = 0;
      ...
      // it("Revert when Batch Liquidate all the WETH Troves", async () => {  
      //     const tokenId = 1;
      ...
      it("Batch Liquidate 1 WETH Troves", async () => {  
          const troveArray = [5];
      ...
      //     console.log('');
      // })
      it("Batch Liquidate 1 USDC Troves", async () => {  
        const troveArray = [17];
