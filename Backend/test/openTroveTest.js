...
const ercToken = require ( "../client/src/artifacts/TestScript/AAAERC.sol/MyToken.json");
  ...
  describe('Zero loan Unit Tests', async function () {
    ...
    // it("should be same as Active Pool name", async () => {
    //   expect( await troveManager1Contract.NAME()).to.equal("TroveManager1");
    // })
    ...
    it("OpenTrove with 5 ETH and 1800 ZUSD", async () => {
      try {
        ...
        await openTrovewithEth(1, eth, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[1].address);
    ...
    it("OpenTrove with 1.5 ETH and 1800 ZUSD", async () => {
      try {
        const zusd = ethers.utils.parseUnits('1800')
        const eth = ethers.utils.parseUnits('1.5')

        const { 0: upperHint, 1: lowerHint } = await hint(zusd,eth);

        await openTrovewithEth(2, eth, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[2].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 1.25 ETH and 1800 ZUSD", async () => {
      try {
        ...
        await openTrovewithEth(3, eth, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[3].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 5 WETH Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(4, tokenId, token);

        await openTrovewithTokens(4, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[4].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 1.5 WETH Token and 1800 ZUSD", async () => {
      try {
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await approval(5, tokenId, token);

        await openTrovewithTokens(5, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[5].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 1.25 WETH Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(6, tokenId, token);

        await openTrovewithTokens(6, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[6].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 5 WBTC Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(7, tokenId, token);

        await openTrovewithTokens(7, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[7].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 1.5 WBTC Token and 1800 ZUSD", async () => {
      try {
        const zusd = ethers.utils.parseUnits('1800')
        const token = ethers.utils.parseUnits('1.5')
        const tokenId = 2;
        ...
        await approval(8, tokenId, token);

        await openTrovewithTokens(8, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[8].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 1.25 WBTC Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(9, tokenId, token);

        await openTrovewithTokens(9, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[9].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 10 WBNB Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(10, tokenId, token);

        await openTrovewithTokens(10, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[10].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 8.5 WBNB Token and 1800 ZUSD", async () => {
      try {
        const zusd = ethers.utils.parseUnits('1800')
        const token = ethers.utils.parseUnits('7.5')
        const tokenId = 3;

        const { 0: upperHint, 1: lowerHint } = await hint(zusd,token);

        await approval(11, tokenId, token);

        await openTrovewithTokens(11, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[11].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 8.25 WBNB Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(12, tokenId, token);

        await openTrovewithTokens(12, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[12].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 150 WSOL Token and 1800 ZUSD", async () => {
      try {
        const zusd = ethers.utils.parseUnits('1800')
        const token = ethers.utils.parseUnits('150')
        const tokenId = 4;

        const { 0: upperHint, 1: lowerHint } = await hint(zusd,token);

        await approval(13, tokenId, token);

        await openTrovewithTokens(13, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[13].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 125 WSOL Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(14, tokenId, token);

        await openTrovewithTokens(14, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[14].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 120 WSOL Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(15, tokenId, token);

        await openTrovewithTokens(15, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[15].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 3000 USDC Token and 1800 ZUSD", async () => {
      try {
        const zusd = ethers.utils.parseUnits('1800')
        const token = ethers.utils.parseUnits('3000')
        const tokenId = 5;

        const { 0: upperHint, 1: lowerHint } = await hint(zusd,token);

        await approval(16, tokenId, token);

        await openTrovewithTokens(16, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[16].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 2750 USDC Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(17, tokenId, token);

        await openTrovewithTokens(17, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[17].address);
        expect(afterBalance).to.equal(zusd);
    ...
    it("OpenTrove with 2500 USDC Token and 1800 ZUSD", async () => {
      try {
        ...
        await approval(18, tokenId, token);

        await openTrovewithTokens(18, tokenId, token, zusd, upperHint, lowerHint);
        let afterBalance = await zusdTokenContract.balanceOf(signers[18].address);
        expect(afterBalance).to.equal(zusd);
