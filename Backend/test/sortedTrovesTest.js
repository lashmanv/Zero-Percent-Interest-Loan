...
const ercToken = require ( "../client/src/artifacts/TestScript/AAAERC.sol/MyToken.json");
  ...
  describe('Zero loan Unit Tests', async function () {
    ...
    describe('Sorted Troves count Sequence Liquidation', async function () {
      before(async () => {
        try {
      ...
      it("OpenTrove with 5 ETH and 1800 ZUSD", async () => {
        ...
        let previous = await sortedTrovesContract.contains(signers[signerId].getAddress(), 0);
        assert.isTrue(previous);

        let after = await sortedTrovesContract.getSize(0);
      ...
      it("OpenTrove with 5 WETH Token and 1800 ZUSD", async () => {
        ...
        let previous = await sortedTrovesContract.contains(signers[signerId].getAddress(), tokenId);
        assert.isTrue(previous);

        let after = await sortedTrovesContract.getSize(tokenId);
        expect(after).to.equal(1);
      ...
      it("Sorted Troves size after 2 ETH Troves liquidation", async () => {  
          const tokenId = 0;
          const zusd = ethers.utils.parseUnits('1800')
          const expected = ethers.utils.parseUnits('2009')

          let previous = await sortedTrovesContract.getSize(tokenId);
          expect(previous).to.equal(3);
      ...
      it("Sorted Troves size after 2 WETH Troves liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
      ...
      it("Sorted Troves size after 2 WBTC Troves liquidation", async () => {  
        const tokenId = 2;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
      ...
      it("Sorted Troves size after 2 WBNB Troves liquidation", async () => {  
        const tokenId = 3;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
      ...
      it("Sorted Troves size after 2 WSOL Troves liquidation", async () => {  
        const tokenId = 4;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
      ...
      it("Sorted Troves size after 2 USDC Troves liquidation", async () => {  
        const tokenId = 5;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
    ...
    describe('Sorted Troves count Random Liquidation', async function () {
      before(async () => {
        try {
