...
const ercToken = require ( "../client/src/artifacts/TestScript/AAAERC.sol/MyToken.json");
  ...
  describe('Zero loan Unit Tests', async function () {
    ...
    async function openTrovewithEth(signerId, ethAmount, zusdAmount, upperHint, lowerHint) {
      ...
      await zusdTokenContract.connect(signers[signerId]).transfer(signers[19].getAddress(), zusdAmount);
    }
    ...
    async function provideToSP(signerId, zusdAmount) {
        await stabilityPoolContract.connect(signers[signerId]).provideToSP(zusdAmount, address);
    }
    ...
    async function price() {
        try {
          ...
          await priceFeedContract.connect(signers[19]).setPrice(price);
        }
        catch(error) {
          console.log(error.reason);
          return(error.reason);
    ...
    describe('Stablility Pool Sequence Liquidation', async function () {
      before(async () => {
        try {
      ...
      it("OpenTrove with 10 ETH and 1800 ZUSD", async () => {
        const signerId = 1;
        const eth = ethers.utils.parseUnits('10')
        ...
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
      ...
      it("OpenTrove with 1.25 ETH and 1800 ZUSD", async () => {
        ...
        await openTrovewithEth(signerId, eth, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(2));
      ...
      it("OpenTrove with 1.15 ETH and 1800 ZUSD", async () => {
        ...
        await openTrovewithEth(signerId, eth, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(3));

        console.log('');
      })

      it("OpenTrove with 10 WETH Token and 1800 ZUSD", async () => {
        const signerId = 4;
        const tokenId = 1;
        ...
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(4));
      ...
      it("OpenTrove with 1.25 WETH Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(5));
      ...
      it("OpenTrove with 1.15 WETH Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(6));

        console.log('');
      })

      it("OpenTrove with 10 WBTC Token and 1800 ZUSD", async () => {
        const signerId = 7;
        const tokenId = 2;
        const token = ethers.utils.parseUnits('10')
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(7));
      ...
      it("OpenTrove with 1.25 WBTC Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(8));
      ...
      it("OpenTrove with 1.15 WBTC Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(9));

        console.log('');
      })

      it("OpenTrove with 20 WBNB Token and 1800 ZUSD", async () => {
        const signerId = 10;
        const tokenId = 3;
        const token = ethers.utils.parseUnits('20')
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(10));
      ...
      it("OpenTrove with 8.5 WBNB Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(11));
      ...
      it("OpenTrove with 8.25 WBNB Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(12));

        console.log(''); 
      })

      it("OpenTrove with 300 WSOL Token and 1800 ZUSD", async () => {
        const signerId = 13;
        const tokenId = 4;
        const token = ethers.utils.parseUnits('300')
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(13));
      ...
      it("OpenTrove with 125 WSOL Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(14));
      ...
      it("OpenTrove with 120 WSOL Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(15));

        console.log('');
      })

      it("OpenTrove with 10000 USDC Token and 1800 ZUSD", async () => {
        const signerId = 16;
        const tokenId = 5;
        const token = ethers.utils.parseUnits('10000')
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(16));
      ...
      it("OpenTrove with 2750 USDC Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(17));
      ...
      it("OpenTrove with 2500 USDC Token and 1800 ZUSD", async () => {
        ...
        await openTrovewithTokens(signerId, tokenId, token, zusd, upperHint, lowerHint);
        let previousBalance = await zusdTokenContract.balanceOf(signers[19].getAddress());
        expect(previousBalance).to.equal(zusd.mul(18));

        console.log('');
      })

      it("Deposit ZUSD Token in Stability Pool", async () => {
        const signerId = 19;
        const zusd = ethers.utils.parseUnits('1800')
        ...
        await provideToSP(signerId, previousBalance);

        let afterBalance = await stabilityPoolContract.getTotalZUSDDeposits();
        expect(afterBalance).to.equal(zusd.mul(18));
      ...
      it("Stability Pool ZUSD balance after 2 ETH Troves liquidation", async () => {  
          const tokenId = 0;
          const zusd = ethers.utils.parseUnits('1800')
          const expected = ethers.utils.parseUnits('2009')

          let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
          expect(previousBalance).to.equal(zusd.mul(18));

          await liquidateinSequence(tokenId, 2);

          let afterBalance = await stabilityPoolContract.getTotalZUSDDeposits()
          expect(afterBalance).to.equal(previousBalance.sub(expected.mul(2)));
      ...
      it("Stability Pool ZUSD balance after 2 WETH Troves liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(2)));
      ...
      it("Stability Pool ZUSD balance after 2 WBTC Troves liquidation", async () => {  
        const tokenId = 2;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(4)));
      ...
      it("Stability Pool ZUSD balance after 2 WBNB Troves liquidation", async () => {  
        const tokenId = 3;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(6)));
      ...
      it("Stability Pool ZUSD balance after 2 WSOL Troves liquidation", async () => {  
        const tokenId = 4;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(8)));
      ...
      it("Stability Pool ZUSD balance after 2 USDC Troves liquidation", async () => {  
        const tokenId = 5;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(10)));
      ...
      it("Stability Pool ZUSD Deposits after liquidation", async () => {  
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('8292')
      ...
      it("Stability Pool Token gain after liquidation", async () => {  
        const zusd = ethers.utils.parseUnits('1800')
        const expected0 = ethers.utils.parseUnits('2.387999999999977200')
        const expected1 = ethers.utils.parseUnits('2.387999999999992935')
        const expected2 = ethers.utils.parseUnits('2.387999999999986770')
        const expected3 = ethers.utils.parseUnits('16.666249999999998715')
        const expected4 = ethers.utils.parseUnits('243.774999999999983114')
        const expected5 = ethers.utils.parseUnits('5223.749999999999973443')

        let afterBalance = await stabilityPoolContract.getDepositorTokenGain(signers[19].getAddress())
        console.log(afterBalance);

        expect(afterBalance[0]).to.equal(expected0);
        expect(afterBalance[1]).to.equal(expected1);
        expect(afterBalance[2]).to.equal(expected2);
        expect(afterBalance[3]).to.equal(expected3);
        expect(afterBalance[4]).to.equal(expected4);
        expect(afterBalance[5]).to.equal(expected5);

        console.log('');
      })

      it("Stability Pool ZQTY gain after liquidation", async () => {  
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('8292')

        let afterBalance = await stabilityPoolContract.getDepositorZQTYGain(signers[19].getAddress())
        console.log(afterBalance);
      ...
      it("Stability Pool withdraw after liquidation", async () => {  
        const zusd = ethers.utils.parseUnits('1800')
        // const ex = ethers.utils.parseUnits('10691.999999999999970000')
        const expected = ethers.utils.parseUnits('8292')
        const expected0 = ('10002395073646960566376')
        const expected1 = BigInt(5000000000000000000000)
        const expected2 = BigInt(5000000000000000000000)
        const expected3 = BigInt(5016000000000000000000)
        const expected4 = BigInt(5024000000000000000000)
        const expected5 = BigInt(5524000000000000000000)

        await price();

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(expected);

        await stabilityPoolContract.connect(signers[19]).withdrawFromSP(previousBalance)

        let afterBalance = await zusdTokenContract.balanceOf(signers[19].getAddress())
        console.log(afterBalance);
        // expect(afterBalance).to.equal(ex);

        let tokenBalance = [];
        
        tokenBalance[0] = await signers[19].getBalance();
        tokenBalance[1] = await ercToken1Contract.balanceOf(signers[19].getAddress())
        tokenBalance[2] = await ercToken2Contract.balanceOf(signers[19].getAddress())
        tokenBalance[3] = await ercToken3Contract.balanceOf(signers[19].getAddress())
        tokenBalance[4] = await ercToken4Contract.balanceOf(signers[19].getAddress())
        tokenBalance[5] = await ercToken5Contract.balanceOf(signers[19].getAddress())

        console.log(tokenBalance)

        // expect(tokenBalance[0]).to.equal(expected0);
        expect(tokenBalance[1]).to.greaterThan(5000000000000000000000);
        expect(tokenBalance[2]).to.greaterThan(5000000000000000000000);
        expect(tokenBalance[3]).to.greaterThan(5016000000000000000000);
        expect(tokenBalance[4]).to.greaterThan(5024000000000000000000);
        expect(tokenBalance[5]).to.greaterThan(5524000000000000000000);
    ...
    describe('Stablility Pool Random Liquidation', async function () {
      before(async () => {
        try {
      ...
      it("Stability Pool ZUSD balance after 1 WETH Trove liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
        ...
        let afterBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(afterBalance).to.equal(previousBalance.sub(expected));
      ...
      it("Stability Pool ZUSD balance after 2 WBTC Troves liquidation", async () => {  
        ...
        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(3)));
      ...
      it("Stability Pool ZUSD balance after 1 WETH Troves liquidation", async () => {  
        const tokenId = 1;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')

        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(5)));
      ...
      //     console.log('');
      // })
      it("Stability Pool ZUSD balance after 1 USDC Troves liquidation", async () => {  
        const tokenId = 5;
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('2009')
      ...
      it("Stability Pool ZUSD balance after 2 WSOL Troves liquidation", async () => {  
        ...
        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(7)));
      ...
      it("Stability Pool ZUSD balance after 1 USDC Troves liquidation", async () => {  
        ...
        let previousBalance = await stabilityPoolContract.getTotalZUSDDeposits()
        expect(previousBalance).to.equal(zusd.mul(18).sub(expected.mul(9)));
      ...
      it("Stability Pool Token gain after liquidation", async () => {  
        const zusd = ethers.utils.parseUnits('1800')
        const expected0 = ethers.utils.parseUnits('2.387999999999977200')
        const expected1 = ethers.utils.parseUnits('2.387999999999977708')
        const expected2 = ethers.utils.parseUnits('2.387999999999993350')
        const expected3 = ethers.utils.parseUnits('16.666249999999994459')
        const expected4 = ethers.utils.parseUnits('243.774999999999996220')
        const expected5 = ethers.utils.parseUnits('5223.749999999999984566')
      ...
      it("Stability Pool ZQTY gain after liquidation", async () => {  
        const zusd = ethers.utils.parseUnits('1800')
        const expected = ethers.utils.parseUnits('42.200546800831993791')
